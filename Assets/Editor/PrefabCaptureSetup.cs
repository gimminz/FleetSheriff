...
public class PrefabCaptureSetup : MonoBehaviour
{
    ...
    public static void CreatePrefabCaptureSystem()
    {
        ...
        // CapturePoint 생성(초기 위치만 지정, 실제 pivot은 시스템 내부에서 보장)
        ...
        // 미리보기 패널 생성 (좌상단)
        GameObject previewPanel = CreatePreviewPanel(canvas.transform);
        RawImage previewImage = previewPanel.GetComponentInChildren<RawImage>();

        // 컨트롤 패널 생성 (우상단) — 스케일 슬라이더만
        ...
        {
            if (s.name.Contains("Scale")) { scaleSlider = s; break; }
        }
        ...
        soCaptureSystem.FindProperty("previewImage").objectReferenceValue = previewImage;
        ...
        Debug.Log("프리팹 캡처 시스템 생성 완료! PrefabCaptureSystem의 Prefab To Capture에 프리팹을 할당하세요.");
    }
    ...
    private static GameObject CreatePreviewPanel(Transform parent)
    {
        ...
        titleRT.sizeDelta = new Vector2(240, 30);

        Text titleText = title.AddComponent<Text>();
        titleText.text = "프리펩 미리보기";
        ...
        previewRT.anchoredPosition = new Vector2(0, -20);
        previewRT.sizeDelta = new Vector2(200, 200);

        RawImage rawImg = preview.AddComponent<RawImage>();
        rawImg.color = new Color(0.3f, 0.3f, 0.3f);

        // 배경(단색) – 필요시 체크 패턴으로 대체 가능
        GameObject bg = new GameObject("Background");
        bg.transform.SetParent(preview.transform, false);
    ...
    private static GameObject CreateControlPanel(Transform parent)
    {
        ...
        // 제목
        CreateLabel(panel.transform, "프리펩 캡처 설정", 20, FontStyle.Bold);

        // 스케일 슬라이더(0.1 ~ 3.0)
        CreateSliderGroup(panel.transform, "Scale", "사이즈", 0.1f, 3f, 1f, wholeNumbers: false);
    ...
    private static void CreateSliderGroup(Transform parent, string name, string label, float min, float max, float value, bool wholeNumbers)
    {
        ...
        // 라벨
        GameObject labelObj = CreateLabel(group.transform, label, 14, FontStyle.Normal);
        ...
        bgRT.sizeDelta = Vector2.zero;
        Image bgImg = bg.AddComponent<Image>();
        bgImg.color = new Color(0.29f, 0.29f, 0.29f);
        ...
        RectTransform fillImgRT = fill.AddComponent<RectTransform>();
        fillImgRT.sizeDelta = Vector2.zero;
        Image fillImg = fill.AddComponent<Image>();
        fillImg.color = new Color(0.29f, 0.56f, 0.89f);
        ...
        handleRT.sizeDelta = new Vector2(15, 0);
    ...
    private static void CreateBackgroundButtons(Transform parent)
    {
        ...
        groupRT.sizeDelta = new Vector2(0, 60);
